...
- Run `detect and run the project's test suite` and `detect and run the project's linter` -- must pass
- Read the epic's `## Verification Contract` and run every required evidence item that remains applicable. If `build` is required, run `detect and run the project's build/package verification when the verification contract requires it`
