...
The polish loop (`drl polish`) generates a standalone bash script that iterates N cycles of: multi-model audit fleet -> polish architect -> inner infinity loop. It runs AFTER the main infinity loop completes, addressing quality gaps across the full priority spectrum (P0 critical through P2 nice-to-have).
...
4. **Inner loop**: Generates and runs a new infinity loop (`drl loop`) to implement the polish epics.
...
drl loop --epics "E1,E2,E3" --reviewers "claude-sonnet,gemini" --force
drl polish --spec-file "docs/specs/my-spec.md" --meta-epic "my-meta-epic" \
