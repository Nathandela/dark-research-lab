...
4. Run baseline quality gates: `detect and run the project's test suite` and `detect and run the project's linter`
   - If `Required evidence` includes `build`, also run `detect and run the project's build/package verification when the verification contract requires it` before spawning reviewers
5. **Lesson-Calibrated Review (LCR)**:
   a. For each reviewer category (security, test-coverage, simplicity, architecture, etc.), run `drl search "<category> review"` to retrieve relevant past lessons
...
6. Search memory with `drl search` for known patterns and recurring issues (broader search beyond per-reviewer calibration)
...
20. Capture novel findings with `drl learn`; pattern-matcher auto-reinforces recurring issues
...
- **security-reviewer**: `drl search "security vulnerability injection XSS"`
- **test-coverage-reviewer**: `drl search "testing coverage TDD mock"`
- **simplicity-reviewer**: `drl search "complexity refactor simplify"`
- **architecture-reviewer**: `drl search "architecture module boundary coupling"`
- **performance-reviewer**: `drl search "performance optimization latency"`
- **design-craft-reviewer**: `drl search "design craft visual hierarchy spacing motion states"`
- **pattern-matcher**: `drl search "pattern recurring mistake"`
...
3. Record resolution via `drl learn` to prevent future contradictions

## Memory Integration
- Run `drl search` before review for known recurring issues
- **LCR**: Per-reviewer calibration queries (see Lesson-Calibrated Review Protocol above)
- **pattern-matcher** auto-reinforces: recurring findings get severity increased via `drl learn`
- **cct-subagent** reads CCT patterns for known Claude failure patterns
- Capture the review report via `drl learn` with `type=solution`
...
- Run `drl knowledge "code review methodology"` for indexed knowledge on review techniques
- Run `drl search "review"` for lessons from past review cycles
...
## Quality Criteria
- Baseline quality gates pass (`detect and run the project's test suite`, `detect and run the project's linter`)
- `detect and run the project's build/package verification when the verification contract requires it` passed when the Verification Contract required build evidence
...
**CRITICAL**: Use `drl learn` for ALL lesson storage -- NOT MEMORY.md.
