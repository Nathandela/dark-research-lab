...
func TestInstallWorkflowCommands(t *testing.T) {
	...
	// Verify files exist
	cmdsDir := filepath.Join(dir, ".claude", "commands", "drl")
...
func TestInstallPhaseSkills(t *testing.T) {
	...
	specDevSkill := filepath.Join(dir, ".claude", "skills", "drl", "spec-dev", "SKILL.md")
...
func TestInstallAgentRoleSkills(t *testing.T) {
	...
	repoAnalyst := filepath.Join(dir, ".claude", "skills", "drl", "agents", "repo-analyst", "SKILL.md")
...
func TestInstallDocTemplates(t *testing.T) {
	...
	readmePath := filepath.Join(dir, "docs", "drl", "README.md")
...
func TestInstallDocTemplates_SubstitutesQualityGates(t *testing.T) {
	...
	workflowPath := filepath.Join(dir, "docs", "drl", "WORKFLOW.md")
...
func TestInstallDocTemplates_UpdatesStaleContent(t *testing.T) {
	...
	docsDir := filepath.Join(dir, "docs", "drl")
...
func TestPruneStaleTemplates_RemovesStaleSkillDirs(t *testing.T) {
	...
	stalePhase := filepath.Join(dir, ".claude", "skills", "drl", "retired-phase")
...
func TestPruneStaleTemplates_RemovesStaleResearchFiles(t *testing.T) {
	...
	staleFile := filepath.Join(dir, "docs", "drl", "research", "retired-topic.md")
	if err := os.WriteFile(staleFile, []byte("# old research\n"), 0644); err != nil {
		t.Fatalf("write stale file: %v", err)
	}

	// Add a stale research directory
	staleDir := filepath.Join(dir, "docs", "drl", "research", "retired-topic")
	if err := os.MkdirAll(staleDir, 0755); err != nil {
