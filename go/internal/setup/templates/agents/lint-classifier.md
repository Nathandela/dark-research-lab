...
- **Class A** (native `rules.json`): The check can be expressed as a regex/glob (`file-pattern`), line count (`file-size`), or shell command (`script`). These map directly to the dark-research-lab rule engine. No external linter needed.
...
CHECK_TYPE must be one of: `file-pattern`, `file-size`, `script` (Class A -- maps to dark-research-lab rule engine), `ast` (Class B -- requires external linter), or `N/A` (not lintable).
