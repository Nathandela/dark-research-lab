...
**Fix**: Already fixed in current `drl loop` output. If running an older generated script, regenerate it with `drl loop --force`.
...
**Fix**: Already fixed in current `drl loop` output. If running an older generated script, regenerate with `drl loop --force`.
...
**Fix**: Already fixed in current `drl loop` output. Dry-run now guards all writes to `loop-execution.jsonl` and all git operations. Regenerate with `drl loop --force` if affected.
...
```bash
drl loop --model 'claude-opus-4-6[1m]' --force
```
...
Regenerate:
```bash
drl loop --force   # New versions include --permission-mode auto
```
...
After upgrading dark-research-lab (`npm update dark-research-lab` or `pnpm update dark-research-lab`), consumers must regenerate their loop script to pick up fixes.

```bash
drl loop --force   # Regenerate with latest fixes
```

The generated `infinity-loop.sh` is a snapshot at generation time. It does NOT auto-update when dark-research-lab is upgraded. Fixes to the loop template (stdout pollution, dry-run guards, reviewer flags) only take effect after regeneration.
