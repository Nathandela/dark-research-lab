...
- **Use comma-separated values for `--epics` and `--reviewers` flags.** The `drl loop` CLI expects comma-separated strings, not space-separated positional arguments.
- **Run from the `go/` directory** (or wherever `go.mod` lives). `drl loop` and `drl polish` won't find the module otherwise.
...
- **Do NOT rely on `npx drl` when the locally-built binary is newer.** The polish loop generates inner loop scripts via `drl loop`. If `npx` resolves a stale version, the generated script may lack critical flags. Ensure the local build is on PATH.
