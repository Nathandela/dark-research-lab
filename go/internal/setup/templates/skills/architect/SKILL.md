...
1. Search memory: \`drl search\` for past features, constraints, decisions
2. Search knowledge: \`drl knowledge "relevant terms"\`
...
1. **Collect results**: Gather all hits from \`drl search\` and \`drl knowledge\` plus any existing documents in \`docs/research/\` relevant to the domain.
...
7. Capture lessons via \`drl learn\`
...
**Invoke \`/compound:launch-loop\` now.** Do NOT attempt to run \`drl loop\` or \`drl polish\` directly. The launch-loop command enforces mandatory skill reading, authorization gates, CLI flag syntax, and critical gotchas. It is the ONLY supported entry point for loop launching.
