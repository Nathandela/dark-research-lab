...
### 1. Query Strategy
For each reviewer in the AgentTeam, the review lead runs a targeted `drl search` query:

```
drl search "<reviewer-domain> <review-context-keywords>"
```

**Examples**:
- Security reviewer on auth code: `drl search "security authentication token session"`
- Test coverage reviewer on storage module: `drl search "testing storage SQLite mock"`
- Architecture reviewer on new module: `drl search "architecture module boundary coupling"`
...
Beyond lessons, reviewers may receive research excerpts from `docs/drl/research/` when directly relevant to the reviewer's domain and the specific work under review.
...
3. **Record**: After resolution, use `drl learn` to either:
...
### No Lessons Found
If `drl search` returns zero results for a reviewer category:
