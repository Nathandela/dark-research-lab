# References: Launch and Growth

## Primary Sources

Read these for deep theoretical grounding on this phase's topics.

| Topic | Paper | Path |
|-------|-------|------|
| Landing page conversion science | Landing Page Conversion Science | `docs/drl/research/b2c_product/landing-page-conversion-science.md` |
| Experimentation and growth analytics | Experimentation and Growth Analytics | `docs/drl/research/b2c_product/market-growth/experimentation-growth-analytics.md` |
| Retention engineering | Retention Engineering and Lifecycle Optimization | `docs/drl/research/b2c_product/market-growth/retention-engineering-lifecycle-optimization.md` |

## Supplementary Sources

Consult these for specific questions or adjacent concerns.

| Topic | Paper | Path |
|-------|-------|------|
| Content SEO product strategy | Content SEO Product Strategy | `docs/drl/research/b2c_product/market-growth/content-seo-product-strategy.md` |
| Go-to-market launch strategy | Go-to-Market Launch Strategy | `docs/drl/research/b2c_product/market-growth/go-to-market-launch-strategy.md` |
| Product-led growth | Product-Led Growth | `docs/drl/research/b2c_product/market-growth/product-led-growth.md` |
| Trust deficit economy | Trust Deficit Economy | `docs/drl/research/b2c_product/trust-deficit-economy.md` |
